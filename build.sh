...
python tools/jsmacro/jsmacro.py -f src/ui-server.js | sed "s#_UI_SERVER_JS_#new Buffer('`base64 -w 0 <src/resources/ui-server.js`','base64')#" | sed "s#_JQUERY_JS_#new Buffer('`base64 -w 0 <src/resources/jquery-1.10.2.min.js`','base64')#" | sed "s/_US_USERS_PASS_/`cat var/postgresql/us_users_pass.txt`/" | sed "s/_US_SESSIONS_PASS_/`cat var/postgresql/us_sessions_pass.txt`/" | sed "s/_US_KEYS_PASS_/`cat var/postgresql/us_keys_pass.txt`/" | sed "s/_US_NODES_PASS_/`cat var/postgresql/us_nodes_pass.txt`/" >target/ui-server.jsmacro.js
